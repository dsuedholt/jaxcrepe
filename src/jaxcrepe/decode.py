...
def argmax(logits):
    ...
    return bins, jaxcrepe.convert.bins_to_frequency(bins)
...
def weighted_argmax(logits):
    ...
    if not hasattr(weighted_argmax, 'weights'):
        weights = jaxcrepe.convert.bins_to_cents(torch.arange(360))
    ...
    # Convert to frequency in Hz
    return bins, jaxcrepe.convert.cents_to_frequency(cents)
