...
def predict_from_file(audio_file,
    ...
    """
    # Load audio
    audio, sample_rate = jaxcrepe.load.audio(audio_file)
...
def infer(frames, model='full', device='cpu', embed=False):
    ...
    if not hasattr(infer, 'model') or not hasattr(infer, 'capacity') or \
       (hasattr(infer, 'capacity') and infer.capacity != model):
        jaxcrepe.load.model(device, model)
...
def postprocess(probabilities,
    ...
    minidx = jaxcrepe.convert.frequency_to_bins(torch.tensor(fmin))
    maxidx = jaxcrepe.convert.frequency_to_bins(torch.tensor(fmax),
                                                torch.ceil)
